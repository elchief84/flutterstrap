...
class _ResponsiveContainerState extends State<ResponsiveContainer> {
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.mode == ContainerMode.containerFluid || breakpoint == Breakpoint.xs) {
      return Row(
    ...
    return Row(
      ...
      children: [
        SizedBox(
          ...
          child: buildContent(breakpoint: breakpoint, containerWidth: containerWidth)
        ),
      ],
    );
  }
  ...
  List<dynamic> buildRows({required Breakpoint breakpoint, required double containerWidth}) {
    final rows = [];
    double rowSize = 0.0;
    ...
    for (var element in widget.children) {
      ...
      if(rowSize == 0) {
        ...
        double widgetWidth = ((cols/widget.layoutColumns!) * containerWidth).floorToDouble();
  ...
  Widget buildContent({required Breakpoint breakpoint, double? containerWidth}) {
